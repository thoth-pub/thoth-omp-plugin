...
OMP 3.5 structured subject values use `name`, `source`, and `identifier`. Plain values can also include a scheme
prefix, such as `THEMA:MFGV` or `93:MFGV`. Without an explicit scheme, the plugin only classifies a value as BIC
or Thema when exactly one validation succeeds. Subjects are synchronized before keywords, duplicate type-and-code
pairs are sent only once, and their Thoth ordinals follow that order.
